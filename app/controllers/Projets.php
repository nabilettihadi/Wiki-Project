...
class Projets extends Controller
{
    ...
    public function delete($id)
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if ($this->projetModel->deleteProjet($id)) {
                ...
                flash('projet_message', 'Something went wrong', 'alert alert-danger');
            }
        }
    ...
    public function edit($id)
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (!$projet || $projet->user_id != $_SESSION['user_id']) {
                flash('projet_message', 'Invalid Projet', 'alert alert-danger');
                redirect('projets');
            }
